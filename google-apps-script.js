...
//    - "Transaction History" with headers: PO Number | Timestamp | Application | From Status | To Status
...
function doGet(e) {
  ...
  try {
    ...
    if (action === 'receive') {
      ...
      historySheet.appendRow([po, ts, 'Receive APP', '-', 'Received, Awaiting Start']);
    ...
    if (action === 'startbuild') {
      ...
      historySheet.appendRow([po, ts, 'Start Build APP', 'Received, Awaiting Start', 'In Assembly']);
    ...
    if (action === 'buildcomplete') {
      ...
      historySheet.appendRow([po, ts, 'Build Complete APP', 'In Assembly', 'Awaiting QC']);
    ...
    if (action === 'qcpass') {
      ...
      historySheet.appendRow([po, ts, 'QC Result APP', 'Awaiting QC', 'QC Passed']);
    ...
    if (action === 'qcfail') {
      ...
      receivedSheet.getRange(row, 8).setValue(ts);
      historySheet.appendRow([po, ts, 'QC Result APP', 'Awaiting QC', 'In Infirmary']);
    ...
    if (action === 'shipout') {
      ...
      historySheet.appendRow([po, ts, 'Ship Out APP', 'QC Passed', 'Tender']);
    ...
    if (action === 'toinfirmary') {
      ...
      var ts = _ts(now);
      receivedSheet.getRange(row, 4).setValue('In Infirmary');
      historySheet.appendRow([po, ts, 'Infirmary APP', currentStatus, 'In Infirmary']);
    ...
    if (action === 'movefrominf') {
      ...
      historySheet.appendRow([po, ts, 'Infirmary APP', 'In Infirmary', newStatus]);
